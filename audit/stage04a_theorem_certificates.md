...
> **2026-09-20 independent-audit amendment.** The original PASS is preserved as historical provenance. C7's cap quantifier and C8's proof-completeness certificate were partially reopened after an external clean-room audit. Their current certified wording is the amended wording below; the re-certification record is `audit/STAGE_04A_INDEPENDENT_AUDIT_RECERTIFICATION_2026-09-20.md`.
...
| C7 | For \(4/9<\rho<2/3\), the unconstrained global response \(R\) contains the rival-exit branch \(U(y)=\delta+2y\). After the source cap, a positive-length \(+2\) segment survives iff \(\phi>\delta\) for \(1/2\le\rho<2/3\), or iff \(\phi>M=\delta/(4\rho-1)\) for \(4/9<\rho<1/2\). Existence of admissible large-cap cases is enough to falsify Proposition 5 as an unqualified global strategic-substitutes theorem. | exact small-cap kill test plus threshold/boundary and large-cap witnesses | N/A | cap may erase the exit branch entirely | independent primitive-payoff candidate maximization + analytic joins | existential counterexample to global substitutability; no universal feasible-positive-slope claim | **PASS — AMENDED** |
| C8 | Complete **pure** Stage-I equilibrium correspondence for source duopoly has the five cap/\(\rho\) cases stated in the manuscript | all BR pieces/cap | analytic exclusion: contraction for \(\rho>2/3\); exact line for \(\rho=2/3\); crossing property plus exhaustive \(x_H<\delta\) / \(x_H\ge\delta\) split for \(4/9<\rho<2/3\) | equality cases \(\phi=s,\phi=\delta/2,\phi=\delta,\phi=R(0)\) retained | completed analytic proof + independent primitive-payoff regression evaluator | no mixed Stage-I classification | **PASS — RECERTIFIED** |
